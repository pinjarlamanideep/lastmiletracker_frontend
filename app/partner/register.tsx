...
import { ArrowLeft, Eye, EyeOff, Mail, Lock, User, Truck } from 'lucide-react-native';
  ...
  const handleRegister = async () => {
    try {
      const res = await fetch(`${API_URL}/api/auth/register`, {
        ...
        body: JSON.stringify({ name, email, password, vehicleNumber, role: 'partner' }),
      });
      const data = await res.json();
      if (!res.ok) {
        console.log('Partner register error', data);
        setSnackType('error');
      ...
      setSnackVisible(true);
      setTimeout(() => router.push('/partner/orders'), 700);
    } catch (err) {
      console.error(err);
    }
  };
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            Join us to start delivering packages
          </Text>
        </View>
